...
public class SortPracticeEveryDay06 {
    ...
    // 插入排序
    // 插入排序是默认当前已经有了一个排好序的队列，然后我们把一个新的数组直接插入到排好序队列的尾部
    // 然后再把这个插入完数据的队列重新排序。实现整个数组的排序。
    // 演化一下流程就是 排好序的队列不断扩充，然后每次插入进来的这个数字都要和原版排好序的每一个书序都比较一下
    // 如果有顺序不对劲的就要给他们调整一下
    ...
    // 归并排序
    // 归并排序就是无限制的递归来分解待排序数组。当递归被分解为一个不能再分解的数的时候，就把两个递归结果合并起来
    // 无限制分解，把数组拆成越来越小的几个数，最后剩一个数的时候，合并，合并的时候谁小先合并谁
    // 递归的精髓还有一个就是每一次递归的结果都会被记录并且返回回去。这样就可以一次递归一次返回然后完成排序
    ...
    // 等于的放在一个区域，这样一圈之后，整体上，就是三块区域：小于，大于，等于。这三块的区域之间就是相对稳定的不需要再改变次序的
