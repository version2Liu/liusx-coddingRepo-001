...
//            System.out.println("第 "+ d + " 轮比较结果" + Arrays.toString(arr));
    ...
    //桶排序
    public static void radixSort9(int[] arr) {
        if (null == arr || arr.length < 2) {
            return;
        }
        int digit = getDigitByArr9(arr);
        int[][] bucket = new int[10][arr.length];
        int[] bucketCount = new int[bucket.length];
    ...
    private static int getDigitByArr9(int[] arr) {
        int max = 0;
        for (int i : arr) {
            max = max > i ? max : i;
        }
    ...
    private static void process10(int[] arr, int l, int r) {
        ...
        int mid = l + ((r - l) >> 1);
        process10(arr, l, mid);
        process10(arr, mid + 1, r);
        mergeLeftAndRight10(arr, l, mid, r);
    }

    private static void mergeLeftAndRight10(int[] arr, int l, int mid, int r) {
        int[] help = new int[r - l + 1];
        int p1 = l;
        int p2 = mid + 1;
        int i = 0;
    ...
    //桶排序
    public static void radixSort10(int[] arr) {
        if (null == arr || arr.length < 2) {
            return;
        }
        int digit = getDigitByArr10(arr);
        int[][] bucket = new int[10][arr.length];
        int[] bucketCount = new int[bucket.length];

        for (int d = 1, n = 1; d <= digit; d++, n = n * 10) {
            //入桶 遍历每一个数字 然后求出当前位数上值 然后入桶
            for (int i = 0; i < arr.length; i++) {
                int bucketIndex = (arr[i] / n) % 10;
                bucket[bucketIndex][bucketCount[bucketIndex]] = arr[i];
                bucketCount[bucketIndex]++;
    ...
    private static int getDigitByArr10(int[] arr) {
        int max = 0;
        for (int i : arr) {
            max = max > i ? max : i;
        }
