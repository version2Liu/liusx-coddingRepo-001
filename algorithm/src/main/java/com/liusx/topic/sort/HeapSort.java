...
//        }
    ...
    /**
     * 大根堆堆插入 index表示的是大根堆中的位置 即当前数字要插入到大根堆的哪一个下标
     * @param arr
     * @param index
     */
    public static void heapInsert(int[] arr, int index){
        //插入的这个数首先找自己的父节点，两者进行比较 如果新插入的数不大于父节点 或者index是0(换句话说就是没有父节点) 就说明当前的插入已完成
        while (arr[index] > arr[(index - 1) / 2]){
            //如果当前节点大于父节点 就交换当前节点 和 父节点位置的值
            swap(arr,index,(index - 1) / 2);
            //交换完了 新插入的数来到了父节点的位置，此时需要继续循环进行判断新插入的数字是否比上面的新的父节点还要大 所以index 需要重新赋值
            index = (index - 1)/2;
        }
    }
